...
class BaseSecurityManager:
    ...
    def _has_access_builtin_roles(
        self, role, action_name: str, resource_name: str
    ) -> bool:
        """
        ...
        for perm in builtin_perms:
            ...
            _action_name = perm[1]
            if re.match(_resource_name, resource_name) and re.match(
    ...
    def _get_user_permission_view_menus(
            ...
            if role.name in self.builtin_roles:
                for resource_name in resource_names:
                    if self._has_access_builtin_roles(
                        role, action_name, resource_name
                    ):
        ...
        resource_names = [
            perm.view_menu.name
            for perm in self.find_roles_permission_view_menus(
                action_name, db_role_ids
            )
        ]
    ...
    def get_user_menu_access(self, menu_names: List[str] = None) -> Set[str]:
        if current_user.is_authenticated:
            return self._get_user_permission_view_menus(
                g.user, "menu_access", resource_names=menu_names
            )
        elif current_user_jwt:
            return self._get_user_permission_view_menus(
